...
class MLMZip():
    ...
    def create_deterministic_masks(self, token_length):
        ...
        num_to_mask = int(token_length * 0.15)  # Calculate the number of tokens to mask based on the desired masking percentage
    ...
    def encode_and_zip(self, text):
        ...
        # zip_package = all_decoded_tokens
        ...
        # zipped_data = self.compress_with_brotli(tensor_bytes)
        # return zipped_data
        ...
        buffer = io.BytesIO()
        torch.save(self.tokens, buffer)
        buffer.seek(0)
        tensor_bytes = buffer.read()

        # compress bytes with brotli
        compressed_bytes = self.compress_with_bz2(tensor_bytes)
        # return compressed_bytes
        ...
        # iterate over the entire list of tokens and replace the tokens with the value in the frequency mapping if the value of the token is greater than the value in the frequency mapping
        # for i in range(len(tokens)):
        #     if str(tokens[i].item()) in self.frequency_mapping and tokens[i].item() > self.frequency_mapping[str(tokens[i].item())]:
        #         tokens[i] = torch.tensor(self.frequency_mapping[str(tokens[i].item())], device=self.device)
        #         cnt += 1
        # print('PRESUM', sum(self.tokens))
        for i in range(len(self.tokens)):
            token_value = self.tokens[i].item()
            mapped_value = self.frequency_mapping.get(str(token_value), token_value)
            if token_value > mapped_value:
                self.tokens[i] = torch.tensor(mapped_value, device=self.device)
                cnt += 1
        ...
        buffer.seek(0)
        tensor_bytes = buffer.read()
        return self.compress_with_brotli(tensor_bytes)
        # print('COUNT', cnt)
        # print("DONE REPLACING")
        # self.tokens = tokens
        # # print('POSTSUM', sum(self.tokens))

        # # take all the tokens and compress them with optpfor
        # codec = getCodec('optpfor')
        # inpComp = np.zeros(self.tokens.size(0), dtype=np.uint32, order='C')
        # compSize = codec.encodeArray(tensor_to_numpy(self.tokens), self.tokens.size(0), inpComp, len(inpComp))
        # compressed_array = inpComp[:compSize]
        # compressed_bytes = compressed_array.tobytes()

        # return compressed_bytes

        # further_compressed = self.compress_with_bz2(compressed_bytes)
    ...
    def encode_tokens(self, text):
        ...
        # i want to concatenate self.tokens with itself 10 times (have 10 copies of the same tensor in one tensor)
    ...
    def unzip_tokens(self, zipped_data):
        zipped_data = self.decompress_brotli(zipped_data)
        ...
        decoded_text = self.tokenizer.decode(loaded_tensor, skip_special_tokens=True)
        decoded_text = decoded_text.replace(' ##', '')

        return decoded_text

    # make a function that compresses text with brotli
    def compress_text_with_brotli(self, text):
        return brotli.compress(text.encode('utf-8'), quality=11)

    def decompress_text_with_brotli(self, zipped_data):
        return brotli.decompress(zipped_data).decode('utf-8')
    ...
    def zip_file(self, input_file, output_file):
        ...
        # zipped_data = self.encode_tokens(text)
        # zipped_data = self.compress_text_with_brotli(text)
        with open(output_file, 'wb') as f:
            f.write(zipped_data)

        # print the size of the zipped data file using os in megabytes
        print("ZIPPED FILE SIZE:", os.path.getsize(output_file) / (1024 * 1024), "MB")
        print("ORIGINAL FILE SIZE:", os.path.getsize(input_file) / (1024 * 1024), "MB")
    ...
    def unzip_file(self, input_file, output_file):
        ...
        decoded_text = self.unzip_tokens(zipped_data)
        # decoded_text = self.decompress_text_with_brotli(zipped_data)
    ...
    def double_compress_with_brotli(self, byte_data):
        compressed_data = brotli.compress(byte_data, quality=11)
        return brotli.compress(compressed_data, quality=11)

    def double_compress_with_bz2(self, byte_data):
        compressed_data = bz2.compress(byte_data)
        return bz2.compress(compressed_data)

    # make a function that compresses text with brotli
    def compress_with_brotli_file(self, filename):
        text = ""
    ...
    def compress_with_bz2_file(self, filename):
        text = ""
        with open(filename, 'r', encoding='utf-8') as f:
            text = f.read()
        return bz2.compress(text.encode('utf-8'))

    # make a function that double compress text with brotli
    def double_compress_with_brotli_file(self, filename):
        text = ""
        with open(filename, 'r', encoding='utf-8') as f:
            text = f.read()
        compressed_text = brotli.compress(text.encode('utf-8'), quality=11)
        return brotli.compress(compressed_text, quality=11)

    # make a function that double compress text with bz2
    def double_compress_with_bz2_file(self, filename):
        text = ""
        with open(filename, 'r', encoding='utf-8') as f:
            text = f.read()
        compressed_text = bz2.compress(text.encode('utf-8'))
        return bz2.compress(compressed_text)
...
if __name__ == "__main__":
    ...
    # brotli_byte_string = mlmzip.compress_with_bz2(mlmzip.compress_with_brotli_file("datasets/1mb.txt"))
    # # save byte string in text.zip
    # with open("text.br", 'wb') as f:
    #     f.write(brotli_byte_string)

    # bz2_byte_string = mlmzip.compress_with_brotli(mlmzip.compress_with_bz2_file("datasets/1mb.txt"))
    # # save byte string in text.zip
    # with open("text.bz2", 'wb') as f:
    #     f.write(bz2_byte_string)

    mlmzip.zip_file("datasets/10mb.txt", "t5-10mb.gpz")
    mlmzip.unzip_file("t5-10mb.gpz", "datasets/AMIAGOD.txt")

    with open("datasets/10mb.txt", 'r', encoding='utf-8') as f:
        text = f.read()
    with open("datasets/AMIAGOD.txt", 'r', encoding='utf-8') as f:
        text2 = f.read()
